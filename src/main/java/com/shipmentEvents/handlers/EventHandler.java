...
public class EventHandler implements RequestHandler<ScheduledEvent, String> {
    ...
    /**
     * Shipment events for a carrier are uploaded to separate S3 buckets based on
     * the source of events. E.g., events originating from
     * the hand-held scanner are stored in a separate bucket than the ones from
     * mobile App. The Lambda processes events from multiple
     * sources and updates the latest status of the package in a summary S3 bucket
     * every 15 minutes.
     * 
     * The events are stored in following format:
     * - Each status update is a file, where the name of the file is tracking number
     * + random id.
     * - Each file has status and time-stamp as the first 2 lines respectively.
     * - The time at which the file is stored in S3 is not an indication of the
    ...
    public String handleRequest(ScheduledEvent scheduledEvent, Context context) {
        ...
        try {
            ...
            logger.log(String.format("Failed to process shipment Updates in %s due to %s", scheduledEvent.getAccount(),
                    ex.getMessage()));
    ...
    private void processShipmentUpdates(final LambdaLogger logger) throws InterruptedException {
        ...
        for (final String bucketName : bucketsToProcess) {
            final List<KeyVersion> filesProcessed = processEventsInBucket(bucketName, logger,
        ...
        EventHandler.getS3Client().putObject(Constants.SUMMARY_BUCKET, summaryUpdateName,
                latestStatusForTrackingNumber.toString());
        ...
        // Before we delete the shipment updates make sure the summary update file
        // exists
    ...
    private List<KeyVersion> processEventsInBucket(String bucketName, LambdaLogger logger,
    ...
    private void deleteProcessedFiles(Map<String, List<KeyVersion>> filesToDelete) {
        ...
        for (Entry<String, List<KeyVersion>> entry : filesToDelete.entrySet()) {
            final DeleteObjectsRequest deleteRequest = new DeleteObjectsRequest(entry.getKey())
                    .withKeys(entry.getValue()).withQuiet(false);
